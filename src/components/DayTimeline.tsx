...
export function DayTimeline({ slots }: Props) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl p-4 shadow-sm">
      <p className="text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-3">Météo de la journée</p>
      <div className="flex gap-2 overflow-x-auto pb-1">
        {slots.map((slot) => (
          <div key={slot.hour} className="flex-shrink-0 flex flex-col items-center gap-1 min-w-[56px]">
            <span className="text-xs text-gray-400 dark:text-gray-500">{slot.label}</span>
            <span className="text-xl">{conditionEmoji(slot)}</span>
            <span className={`text-sm font-semibold ${tempColor(slot.temp)}`}>{slot.temp}°</span>
