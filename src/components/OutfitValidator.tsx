...
export function OutfitValidator({ onFeedback, todayEntry }: ValidatorProps) {
  ...
  return (
    <div className="bg-gray-50 border border-gray-100 rounded-2xl p-4">
      <p className="text-xs text-gray-400 text-center mb-3">J'ai porté cette tenue aujourd'hui…</p>
      <div className="flex gap-2">
        {FEEDBACK_OPTIONS.map((opt) => (
          <button
            key={opt.value}
            onClick={() => onFeedback(opt.value)}
            className="flex-1 flex flex-col items-center gap-1 py-2 rounded-xl border border-gray-200 hover:border-blue-300 hover:bg-blue-50 transition-colors text-sm"
          >
